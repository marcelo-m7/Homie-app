...
from security import (
    csrf_protect, generate_csrf_token, safe_delete_item, 
...
# Error handlers - return generic messages, log details server-side
@app.errorhandler(403)
def forbidden_error(error):
    logger.warning(f"403 Forbidden: {request.url} - User: {session.get('user_id', 'Anonymous')}")
...
def internal_error(error):
    logger.error(f"500 Internal Error: {str(error)} - URL: {request.url}")
...
def add_shopping_item():
    try:
        item_name = sanitize_html_input(request.form['item_name'].strip())
...
def delete_shopping_ajax(item_id):
    try:
        ...
        # Use safe delete with ownership check
        if safe_delete_item(conn, 'shopping_items', item_id, user_id):
            conn.commit()
            conn.close()
            return jsonify({'success': True})
        else:
            conn.close()
            log_security_event("unauthorized_delete_attempt", f"shopping_item:{item_id}", user_id)
            return jsonify({'success': False, 'error': 'Unauthorized'}), 403
            
    except Exception as e:
        logger.error(f"Error deleting shopping item: {str(e)}")
...
def toggle_shopping_item(item_id):
    try:
        ...
        # Get current item with ownership check
        item = conn.execute('SELECT * FROM shopping_items WHERE id = ? AND added_by = ?', (item_id, user_id)).fetchone()
        if not item:
            conn.close()
            log_security_event("unauthorized_toggle_attempt", f"shopping_item:{item_id}", user_id)
...
def delete_chore_ajax(chore_id):
    try:
        ...
        # Use safe delete with ownership check
        if safe_delete_item(conn, 'chores', chore_id, user_id):
            conn.commit()
            conn.close()
            return jsonify({'success': True})
        else:
            conn.close()
            log_security_event("unauthorized_delete_attempt", f"chore:{chore_id}", user_id)
            return jsonify({'success': False, 'error': 'Unauthorized'}), 403
...
def delete_expiry_ajax(item_id):
    try:
        ...
        # Use safe delete with ownership check
        if safe_delete_item(conn, 'expiry_items', item_id, user_id):
            conn.commit()
            conn.close()
            return jsonify({'success': True})
        else:
            conn.close()
            log_security_event("unauthorized_delete_attempt", f"expiry_item:{item_id}", user_id)
            return jsonify({'success': False, 'error': 'Unauthorized'}), 403
            
    except Exception as e:
        logger.error(f"Error deleting expiry item: {str(e)}")
        return jsonify({'success': False, 'error': 'An error occurred'}), 500
...
def delete_bill_ajax(bill_id):
    try:
        ...
        # Use safe delete with ownership check
        if safe_delete_item(conn, 'bills', bill_id, user_id):
            conn.commit()
            conn.close()
            return jsonify({'success': True})
        else:
            conn.close()
            log_security_event("unauthorized_delete_attempt", f"bill:{bill_id}", user_id)
            return jsonify({'success': False, 'error': 'Unauthorized'}), 403
